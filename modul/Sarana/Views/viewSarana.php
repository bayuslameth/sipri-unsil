            ...
            <div class="d-md-flex justify-content-between align-items-center dt-layout-end col-md-auto ms-auto mt-0">
                <div class="dt-buttons btn-group flex-wrap mb-0">
                    <button class="btn create-new btn-primary" type="button" onclick="add()"><span><span
                                ...
                                <span class="d-none d-sm-inline-block">Tambah Fasilitas</span></span></span></button>
                </div>
            </div>
        </div>
        <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" id="nama_fasilitas" name="nama_fasilitas" class="form-control"
                                placeholder="Contoh: Ruangan Laboratorium">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" id="kondisi" name="kondisi" class="form-control"
                                placeholder="Jelaskan kondisi Fasilitas">
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
            </div>
...
function add() {
    ...
    $('#modal .modal-title').text('Buat Informasi Fasilitas');
    modal.modal('show');
}
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status) {
                ...
                $('#id').val(d.id);
                $('#nama_fasilitas').val(d.nama_fasilitas);
                $('#tipe').val(d.tipe);
                $('#kondisi').val(d.kondisi);

                $('#modalLabel').text('Edit Informasi Fasilitas');
                modal.modal('show');
            }
        },
        error: function(jqXHR, textStatus, errorThrown, exception) {
...
function remove(id, nama_fasilitas) {
    bootbox.confirm({
        title: "<strong>Hapus Data Fasilitas</strong>",
        message: "Yakin akan menghapus data Fasilitas <strong style='text-decoration: underline;'>" + nama_fasilitas +
                    ...
                    success: function(response) {
                        if (response.status) {
                            showNotify({
                                status: 'success',
                                title: 'Berhasil',
                                text: "Informasi Fasilitas '" + response.nama_fasilitas +
                                    "' telah dihapus",
                                type: 'fill'
                            });
