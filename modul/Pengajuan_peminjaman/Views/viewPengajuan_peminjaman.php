    ...
    <div class="card">
        <div class="row card-header flex-column flex-md-row">
            <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto mt-0">
                <h5 class="card-title mb-0 text-md-start text-center text-dark">Pengajuan Peminjaman</h5>
            </div>
        </div>
        <div class="card-body">
...
document.addEventListener("DOMContentLoaded", function() {
    table = $('#table').DataTable({
        ...
        order: [],
        ajax: {
            url: '/pengajuan-peminjaman/datatable',
            method: 'POST',
        },
        columns: [{
                data: 'no',
...
function edit(id) {
    $.ajax({
        type: "POST",
        url: "/pengajuan-peminjaman/getData",
        data: {
            id: id
        },
        dataType: "JSON",
...
function approve(id, nama) {
    bootbox.confirm({
        title: "<strong>Setujui peminjaman fasilitas</strong>",
        message: "Yakin akan menyetujui peminjaman yang diajukan oleh <strong style='text-decoration: underline;'>" + nama +
            "</strong>?",
        closeButton: false,
        centerVertical: true,
        size: 'small',
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url: "/pengajuan-peminjaman/approve",
                    type: "POST",
                    dataType: "JSON",
                    data: {
                        id: id,
                    ...
                    success: function(response) {
                        if (response.status) {
                            ...
                            showNotify({
                                status: 'warning',
                                title: 'Gagal',
                                text: "Peminjaman tidak berhasil disetujui.",
                                type: 'fill'
                            });
                        }
                    },
...
function reject(id, nama) {
    bootbox.confirm({
        title: "<strong>Tolak peminjaman fasilitas</strong>",
        message: "Yakin akan menolak peminjaman yang diajukan oleh <strong style='text-decoration: underline;'>" + nama +
            "</strong>?",
        closeButton: false,
        centerVertical: true,
        size: 'small',
        ...
        callback: function(result) {
            if (result) {
                $.ajax({
                    url: "/pengajuan-peminjaman/reject",
                    type: "POST",
                    dataType: "JSON",
                    data: {
                        id: id,
                    ...
                    success: function(response) {
                        if (response.status) {
                            ...
                            showNotify({
                                status: 'warning',
                                title: 'Gagal',
                                text: "Peminjaman tidak berhasil ditolak.",
                                type: 'fill'
                            });
                        }
                    },
